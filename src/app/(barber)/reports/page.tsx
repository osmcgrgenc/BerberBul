...
export default async function ReportsPage() {
  ...
  const { data: appointments } = await supabase
    .from("appointments")
    .select(
      "id, appointment_time, customer_id, status, services ( name, price )"
    )
  ...
  const totalRevenue = completed.reduce(
    (sum, a) => sum + (a.services?.[0]?.price || 0),
    0
  );
  ...
  completed.forEach((a) => {
    ...
    val.total += a.services?.[0]?.price || 0;
    val.count += 1;
  ...
  completed.forEach((a) => {
    ...
    val.revenue += a.services?.[0]?.price || 0;
    val.count += 1;
    dayMap.set(day, val);
  });
  const trendData = Array.from(dayMap.values()).sort((a, b) =>
